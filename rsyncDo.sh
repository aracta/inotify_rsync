#!/bin/bash
SRC_DIR="/home/wwwroot/u1.elabscience.cn"  #注意末尾不含/
#TAR_DIR="/home/wwwroot/u1.elabscience.cn"  #注意末尾不含/
...
do
	echo $(date "+%Y-%m-%d %H:%M:%S") " Translating ${SRC_DIR}${DERECTORY} to ${TAR_DIR}${DERECTORY}.." >> $TRANSLATING_LOG 2>&1
	#注意rsync不要使用-r参数，避免发生递归，影响效率
	#rsync -PdztopgDl --delete ${SRC_DIR}${DERECTORY} ${TAR_DIR}${DERECTORY} 2>> $ERR_LOG #错误重定向
